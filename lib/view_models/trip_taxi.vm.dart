...
import 'package:mahilasaarthi/models/payment_method.dart';
import 'package:mahilasaarthi/models/vehicle_type.dart';
import 'package:mahilasaarthi/requests/payment_method.request.dart';
import 'package:mahilasaarthi/requests/taxi.request.dart';
import 'package:mahilasaarthi/view_models/taxi_google_map.vm.dart';
...
//get current on going trip
  void getOnGoingTrip() async {
    ...
    try {
      ...
      print("trip ongoing error 1 ==> $error");
    }
  ...
  void cancelTrip() async {
    ...
    try {
      ...
      print("trip ongoing error 2==> $error");
    }
    setBusyForObject(onGoingOrderTrip, false);
  }
  ...
  loadTripUIByOrderStatus({bool initial = false}) {
    ...
    //
    // }catch(e){
    //   toastError("Order Cancellation".tr());
    //   setCurrentStep(1);
    //   clearMapData();
    //   stopAllListeners();
    //   closeOrderSummary();
...
//
  void startHandlingOnGoingTrip() async {
    ...
    tripUpdateStream = firebaseFirestore
        ...
        .listen(
      (event) async {
        print("DRIVER EVENT : " + event.id.toString());
        print("DRIVER EVENT : " + event.data() .toString());
        ...
        if (driverId != null && onGoingOrderTrip?.driverId == null) {
          onGoingOrderTrip?.driverId = int.parse(event.data()!["driver_id"].toString());
      ...
      print("object1213");
      print("object1213 " + e.toString());
    }
    //start order details listening stream
  }

  //DRIVER SECTION
  loadDriverDetails(mDriverId) async {
    try {
    //   final mDriverId = onGoingOrderTrip?.driverId;
      ...
      if (onGoingOrderTrip?.driver == null) {
        ...
        if (onGoingOrderTrip?.driver == null) {
          ...
          loadDriverDetails(onGoingOrderTrip?.driverId);
        }
      }
      notifyListeners();
    } catch (error) {
      print("trip ongoing error 3==> $error");
    }
  }
